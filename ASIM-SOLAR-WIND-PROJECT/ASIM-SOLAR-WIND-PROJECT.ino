...
void loop()
{
    ...
    Wind_Voltage = Voltage(Wind_Volt_Pin);
    Solar_Voltage = Voltage(Solar_Volt_Pin);
    bat_Voltage = Voltage(Sync_Volt_Pin);
    Solar_Current = Current(Solar_Cur_Pin);
    Wind_Current = Current(Wind_Cur_Pin);
    bat_Current = Current(Sync_Cur_Pin);
...
// OR SIMPLIFY AS
  //  value = (Raw1 * 0.0049);    // where R2 = 7500 , R1 = 30000
  //  Volts1 = (value / 0.2) + 0.2;    // R2 / R1 + R2 = 0.2  , 5 / 1024 = 0.0049
...
double Current(int pin)
{
  ...
  Voltage = ( Voltage / 25);
  Current = ((Voltage - 2.5) / 0.0066); // Sensed voltage is converter to current   here offset volt = 2.49 because at 0A , its gives 2.49v and 66 is factor for 30A ACS712 sesnor
  return Current;
}
...
//  lcd1.print("WIND PARAMETERS");
  lcd1.setCursor(0, 1);
  lcd1.print("V:"+String(Wind_Voltage,1)+" V  I:"+String(Wind_Current)+" A ");
  
  
//  lcd2.print("SOLAR PARAMETERS");
  lcd2.setCursor(0, 1);
  lcd2.print("V:"+String(Solar_Voltage,1)+" V  I:"+String(Solar_Current)+" A ");
...
//  lcd4.print("BAT ");
  lcd4.setCursor(0, 1);
  lcd4.print("V:"+String(bat_Voltage,1)+" V  I:"+String(bat_Current)+" A ");
}
